        ...
        {skills_images.map((skill) => (
          ...
          >
            <motion.div
              ...
              transition={{
                delay: 0.1 * (skills_images.indexOf(skill) * 0.1),
                duration: 0.2,
              }}
            >
              <Image
              ...
              />
              <div
                className={`group-hover:scale-100 scale-50 group-hover:opacity-100 group-hover:bg-sand-140 rounded-md px-2 opacity-0 absolute bottom-2 -right-1 block shadow-lg border border-base-300 group-hover:z-20 text-sm transition-all ${isTouchDevice ? "touch-hover" : ""}`}
              >
                {skill.title}
              </div>
            </motion.div>
